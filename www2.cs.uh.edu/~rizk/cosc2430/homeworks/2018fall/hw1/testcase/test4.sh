...
diff -iEBwu ans3.txt ${output}3.txt > 3.diff
if [ $? -ne 0 ]; then
...
# For test case 4
timeout -k 5s 5s ./${programname} "A=${input1}4.txt;B=${input2}4.txt;C=${output}4.txt" 1>1.stdcout 2>1.stderr
diff -iEBwu ans4.txt ${output}4.txt > 4.diff
if [ $? -ne 0 ]; then
